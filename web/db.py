...
class DB: 
    ...
    def query(self, sql_query, vars=None, processed=False, _test=False): 
        ...
        if db_cursor.description:
            if self.dbtype=='oracle' and self.forcelower: #20150126 dongyg: Oracle TableName/FieldName convert to lowercase
                names = [x[0].lower() for x in db_cursor.description]
            else:
                names = [x[0] for x in db_cursor.description]
            if self.dbtype == 'mssql':
                L = [x for x in db_cursor.fetchall()]
                gen = (storage(dict(zip(names,L.pop(0)))) for i in xrange(len(L)))
                out = iterbetter(gen)
                out.list = lambda: [x for x in gen]
            else:
                def iterwrapper():
